...
import { PushNotificationEmailService } from 'services';
...
const Notice = (props) => {
    ...
    const onSubmit = ({ notice_input_title, notice_input_url, notice_input_contents}) => {
        ...
        PushNotificationEmailService.addPushNotification(isoDate, notice_input_title, notice_input_url, notice_input_contents).then(response => {
            if (response.code == 200) {
                getPushNotification(activePage, perPage);
                Notification.success(t('msg_push_notice_success'));
                // setMsgSuccess(t('msg_push_notice_success'));
            } else {
                Notification.error(t('msg_push_notice_error'));
                // setError('apiError', { message: t('msg_push_notice_error') || error });
            }
        });
    ...
    const handleNoticeHistoryDeleteSave = () => {
        ...
        PushNotificationEmailService.deletePushNotification(currentPushId).then(response => {
            if (response.code == 200) {
                getPushNotification(activePage, perPage);
                Notification.success(t('msg_push_notice_success'));
                // setMsgDeletePush({success: t('msg_push_notice_success')});
                setIsOpenPopupDelete(false);
            } else {
                Notification.error(t('msg_push_notice_error'));
                // setMsgDeletePush({error: t('msg_push_notice_error')});
            }
        });
        setIsOpenPopupDelete(false);
    }
    ...
    const handleNoticeHistoryEditSave = ({description}) => {
        ...
        PushNotificationEmailService.editPushNotification(currentPushId, description).then(response => {
            if (response.code == 200) {
                getPushNotification(activePage, perPage);
                Notification.success(t('msg_push_notice_success'));
                // setMsgEditPush({success: t('msg_push_notice_success')});
            } else {
                Notification.error(t('msg_push_notice_error'));
                // setMsgEditPush({error: t('msg_push_notice_error')});
            }
        });
        setIsOpenPopupEdit(false);
        return true;
            ...
            <main className="main-page main-page--push-notification">
                ...
                <section className={`wrapper ${isActiveSideBar ? "active" : ""}`}>
                    ...
                    <div className="page__heading">
                        <h2 className="page__title mb-0">{t('notice_page_heading')}</h2>
                    </div>
...
export default withTranslation()(withRouter((Notice)));
