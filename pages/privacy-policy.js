...
import { userService, PrivacyService } from '../services'
...
const Policy = (props) => {
    ...
    // show hide edit content HTML
    const [isEdit, setIsEdit] = useState(false);
    const [termDescription, setTermDescription] = useState();
    const getPrivacy = () => {
        PrivacyService.getPrivacy().then((response) => {
    ...
    useEffect(() => {
        if (quill) {
            const html = termDescription ? termDescription : defaultHtml;
    ...
    const handleSubmitEditor = () => {
        ...
        if (quill) {
            PrivacyService.editPrivacy(quill.root.innerHTML).then(response => {
                //console.log(`quill.root.innerHTML `, response);
                if (response.code == 200) {
                    Notification.success(t('msg_update_success'));
                    getPrivacy();
                } else {
            ...
            Notification.warning(t('msg_warning'));
        }
    };

    return (
        <RouteGuard>
            <div className="main-page main-page--privacy_policy">
                <Sidebar isMenuActive="privacy_policy" isActiveSideBar={isActiveSideBar} onClick={handleClickSideBar}/>
                ...
                <section className={`wrapper ${isActiveSideBar ? "active" : ""}`}>
                    ...
                    <div className="content content--no-title">
                        <div className="content__inner">
                            ...
                            <div className="terms terms--privacy-policy relative">
                                {user?.permission == 'admin' &&
                                <div className="terms__actions f jce aic">
                                    <button type="button"
                                            className="popup__btn-edit btn btn-default btn--no-shadow btn--small"
                                            onClick={e => handleShowEditor(e)}>{t('btn_edit_policy')}</button>
                                    <button type="submit"
                                            className="popup__btn-save btn btn-gradient btn--no-shadow btn--small"
                                            onClick={e => handleSubmitEditor(e)}>{t('btn_save_policy')}</button>
                                </div>
                                }

                                <div className={isEdit ? 'quill is-active' : 'quill'}>
                                    <div ref={quillRef} />
                                </div>

                                <div className={!isEdit ? 'terms__html wyswyg' : 'terms__html wyswyg is-active'}>
                                    {termDescription ? htmlReactParse(termDescription) : htmlReactParse(defaultHtml)}
                                </div>
                                <div className="terms__footer f jce aic mt-5 mb-5">以上</div>
                            </div>
...
const defaultHtml = `<div className="privacy-policy wyswyg">
    <h1>プライバシーポリシー</h1>
    <p>当方は，提供するサービス「Ｂｙｍｅ」（以下,「本サービス」といいます。）における，ユーザーの個人情報の取扱いについて，以下のとおりプライバシーポリシー（以下，「本ポリシー」といいます。）を定めます。</p>
    ...
    <ol>
        <li>
            <p>当方は，本人から個人情報の開示を求められたときは，本人に対し，遅滞なくこれを開示します。ただし，開示することにより次のいずれかに該当する場合は，その全部または一部を開示しないこともあり，開示しない決定をした場合には，その旨を遅滞なく通知します。なお，個人情報の開示に際しては，1件あたり1，000円の手数料を申し受けます。</p>
            <ul>
    ...
    <p>住所：神奈川県横浜市４－１８－６グリーングラスＢ１０２</p>
    <p>屋号：Ｂｙｍｅ</p>
    <p>Eメールアドレス：<a href="mailto:info@app-byme.com">info@app-byme.com</a></p>
...
export default withTranslation()(withRouter((Policy)));
